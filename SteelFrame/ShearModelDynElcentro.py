...
def gen_DynSettings() -> NoReturn:
    content = f'set dataDir F:/data_RSRP/SteelFrame/ShearModelDynElcentro;\n' \
              f'set path_GMR F:/data_RSRP/SteelFrame/ShearModelDynElcentro/GMR.txt;\n' \
              f'set dt {dt};\n' \
              f'set npts {npts};\n' \
              f'set factor {factor};\n'
...
if __name__ == '__main__':
    path_log = 'Logs.txt'
    path_GMR = 'F:/data_RSRP/SteelFrame/ShearModelDynElcentro/RSN6_IMPVALL.I_I-ELC180.AT2'
    data = []
    ag, npts, dt = read_AT2(path_GMR)
    PGA = max(ag)
    # factor = 0.7 * G / PGA
    factor = 1
    np.savetxt('F:/data_RSRP/SteelFrame/ShearModelDynElcentro/GMR.txt', ag)
    msg_list = simulation()
