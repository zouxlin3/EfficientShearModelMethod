...
def visualization(floor: int, disp: np.ndarray, force: np.ndarray, disp_: np.ndarray, force_: np.ndarray) -> NoReturn:
    ...
    save_path = f'F:/data_RSRP/SteelFrame3/ShearModel/InterStory_{floor}.png'
    cycle_compare(disp, force / kN, disp_, force_ / kN, ['Fiber model', 'Shear model'], save_path)


if __name__ == '__main__':
    ESDOF1 = read_json('F:/data_RSRP/SteelFrame3/ShearModel/floor1.json')
    ESDOF2 = read_json('F:/data_RSRP/SteelFrame3/ShearModel/floor2.json')
    ESDOF3 = read_json('F:/data_RSRP/SteelFrame3/ShearModel/floor3.json')
    ESDOF = [ESDOF1, ESDOF2, ESDOF3]
    data = pd.read_csv('F:/data_RSRP/SteelFrame3/SPO/results.csv')
    res_save_path = f'F:/data_RSRP/SteelFrame3/ShearModel/result.csv'
    floors = 3
    data_dict = {}
